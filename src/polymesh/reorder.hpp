...
namespace fvm
{
    ...
    {
        Index nRows = 0;
        std::vector<Index> rowPtr; // Row pointers (size: nRows + 1)
        std::vector<Index> colIdx; // Column indices

        SparseMatrix() = default;
        SparseMatrix(Index n) : nRows(n), rowPtr(n + 1, 0) {}

        /// Returns the degree (number of neighbors) for a given row
        Index degree(Index row) const
        {
            return rowPtr[row + 1] - rowPtr[row];
        }

        /// Returns iterators to the neighbors of a given row
        auto neighbors(Index row) const
        {
    ...
    public:
         ...
         */
        virtual std::vector<Index> getOrder(const PolyMesh &mesh) = 0;
    };
    ...
    {
    public:
        std::vector<Index> getOrder(const PolyMesh &mesh) override;
    };
     ...
     */
    FVM_API SparseMatrix buildCellAdjacency(const PolyMesh &mesh, Index numCells = 0);
